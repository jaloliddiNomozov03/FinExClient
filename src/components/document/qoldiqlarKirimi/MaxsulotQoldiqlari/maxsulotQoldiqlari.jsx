...
import {deleteEnteringGoods, getEnteringGoodsPage} from "../../../../server/config/document/EnteringGoodsService";
...
const MaxsulotQoldiq = () => {
  ...
  const columns = [
    ...
    { title: "Izox", dataIndex: "comment", key: "comment" },
    {
      ...
      key: "x",
      width: "10%",
      render: (record) => <Button onClick={()=>deleteEnteringGoodsById(record)} type="primary">Delete</Button>,
    },
  ];
  const TitleHeader = ()=><HeaderMaxsulot getEnteringGoods={getEnteringGoods}/>;
  return (
    <Table
      className="Jadval"
      columns={columns}
