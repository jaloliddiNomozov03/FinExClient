...
import {deleteEnteringDebtors, getEnteringDebtorsPage} from "../../../../server/config/document/EnteringDebtorsService";
...
const Qarizdorlik = () => {
    ...
    getEnteringDebtorsPage(currentPage, pageSize).then(value => {
      if (value && value.data && value.data.content){
        ...
        for (let i = 0; i < value.data.content.length; i++) {
          ...
          enteringDebtors.responsibleName = value.data.content[i].responsibleName;
  ...
  const columns = [
    ...
    { title: "Comment", dataIndex: "comment", key: "comment" },
    {
      ...
      key: "x",
      width: "10%",
      render: (record) => <Button type="primary" onClick={()=>deleteEnteringDebtorsById(record)}>Delete</Button>,
    },
  ];
  const TitleHeader=()=><HeaderQarizdorlik getEnteringDebtors={getEnteringDebtors}/>;
  return (
    <Table
      className="Jadval"
      columns={columns}
