...
import {deleteEnteringCreditors, getEnteringCreditorsPage} from "../../../../server/config/document/EnteringCreditorsService";
...
const XodimlarQoldiqi = () => {
    ...
    getEnteringCreditorsPage(currentPage, pageSize).then(value => {
      if (value && value.data && value.data.content){
        ...
        for (let i = 0; i < value.data.content.length; i++) {
          ...
          enteringCreditors.responsibleName = value.data.content[i].responsibleName;
  ...
  const columns = [
    ...
    { title: "ResponsibleName", dataIndex: "responsibleName", key: "responsibleName", width: "25%" },
    { title: "KonstantaDP", dataIndex: "isConstanta", key: "isConstanta", width: "10%",
    ...
    { title: "Comment", dataIndex: "comment", key: "comment" },
    {
      ...
      key: "x",
      width: "10%",
      render: (record) => <Button type="primary" onClick={()=>deleteEnteringCreditorsById(record)}>Delete</Button>,
    },
  ];
  const TitleHeader=()=><HeaderXodimlar getEnteringCreditors={getEnteringCreditors}/>;
  return (
    <Table
      className="Jadval"
      columns={columns}
